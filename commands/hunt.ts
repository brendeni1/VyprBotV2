...
module.exports = async (client, context) => {
  if(cooldown.huntCheck(context.user)) {
    return { success: false, reply: `You're on cooldown! Wait 1 hour in between hunting.` }
  }
  ...
  if(nammers == null) {
    ...
    cooldown.huntAdd(context.user) ; setTimeout(() => { cooldown.huntDelete(context.user) }, 3600000)
    return { success: true, reply: `You're a new user! Here's 50 nammers to get you started! You now have 50 nammers.` }
  }
  nammers = +nammers
  ...
  if(nammers + amount < 0) {
    ...
    cooldown.huntAdd(context.user) ; setTimeout(() => { cooldown.huntDelete(context.user) }, 3600000)
    return { success: true, reply: `${message} | You have 0 nammers.` }
  }
  await utils.setData(`${context.user}Nammers`, nammers + amount)
  cooldown.huntAdd(context.user) ; setTimeout(() => { cooldown.huntDelete(context.user) }, 3600000)
  return { success: true, reply: `${message} | You have ${nammers + amount} nammers.` }
}