...
module.exports = async (client, context) => {
  ...
  try {
    ...
    if (!locationData.items[0]) { return { success: false, reply: `The location provided to the geocoding API was invalid.` } }
    const country = locationData.items[0].address.countryCode
    const covidStats = await utils.fetch(`https://coronavirus-monitor-v2.p.rapidapi.com/coronavirus/latest_stat_by_country.php?country=${country}`, { "headers": { "x-rapidapi-host": "coronavirus-monitor-v2.p.rapidapi.com", "x-rapidapi-key": process.env['COVID_KEY'] } })
