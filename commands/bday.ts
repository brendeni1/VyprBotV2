...
module.exports = async (client, context) => {
	...
	if (context.args[0]) {
		context.user = context.args[0].toLowerCase().replace('@', '')
	}
	if (context.user == 'vyprbot') {
		return {
			success: true,
			reply: `I was made on November 12, 2021 which was ${utils.formatDelta('November 12 2021')} ago.`
		}
	}
	...
	return {
		success: true,
		reply: `${userContext.userAddress} born on ${bday}, ${userContext.pronoun} are ${currentage} years old and are turning ${turningage} on ${bday.replace(/(160[0-9]|16[1-9][0-9]|1[7-9][0-9]{2}|[2-9][0-9]{3})/, new Date().getFullYear() + yearCaseBday)} which is in ${nextBday}.`
	}
}