...
module.exports = async (client, context) => {
    ...
    try {
      ...
      const address = user.toLowerCase() == context.user ? { name: `You`, verb: 'are' } : { name: `@${user}`, verb: 'is' }
      if (!followData.followedAt) { return { success: true, reply: `${address.name} ${address.verb} not following @${channel}.` } }
