...
module.exports = async (client, context) => {
  if (!context.args[0]) { return { success: false, reply: `Please provide a song name to look up.` } }
  const searchInput = context.args.join(' ').match(/index(:|=)(\d+)/i)
  var index = searchInput ? +searchInput[2] : 0; if(searchInput) { context.args.splice(context.args.indexOf(searchInput[0]), 1) }
  ...
  if (tracks[index].track.has_lyrics == 1) { flags.push('Has Lyrics') }
  if (tracks[index].track.has_subtitles == 1) { flags.push('Has Subtitles') }
