...
module.exports = async (client, context) => {
  if (!context.args[0]) { return { success: false, reply: "Please provide a phrase to look up!" } }
  const phraseCheck = context.args.join(' ').match(/index(:|=)(\d+)/i)
  var index = phraseCheck ? +phraseCheck[2] : 0; if(phraseCheck) { context.args.splice(context.args.indexOf(phraseCheck[0]), 1) }
  let urbanResult = await utils.fetch(`https://api.urbandictionary.com/v0/define?term=${encodeURIComponent(context.args.join(' '))}`, { timeout: 10000 })
